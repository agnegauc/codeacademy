...
app.get("/users/cars/:brand", (req, res) => {
  const { brand } = req.params;
  const usersWithSelectedCar = data.filter((user) => user.car === brand);

  res.send(usersWithSelectedCar).end();
});

app.get("/users/:id", (req, res) => {
  const { id } = req.params;
  //   const user = data.filter((user) => user.id === +id); // with filter we get an array, with find - just the object
...
app.get("/females", (_, res) => {
  const femaleUsers = data
    .filter((user) => user.gender === "Female")
    .map((user) => `${user.first_name} ${user.last_name}`);

  res.send(femaleUsers).end();
});
